...
CastPlayer.prototype.initializeCastPlayer = function() {
    ...
    options.receiverApplicationId = appId ? appId : 'D2886552';
...
var PlayerHandler = function(castPlayer) {
    ...
    this.load = function(mediaIndex) {
        ...
        document.getElementById('media_subtitle').innerHTML =
            castPlayer.mediaContents[castPlayer.currentMediaIndex]['author'];
...
CastPlayer.prototype.setupLocalPlayer = function () {
    ...
    playerTarget.load = function(mediaIndex) {
        localPlayer.src =
            this.mediaContents[mediaIndex].prog;
        localPlayer.load();
    }.bind(this);
...
CastPlayer.prototype.setupRemotePlayer = function () {
    ...
    playerTarget.load = function (mediaIndex) {
        ...
        var mediaInfo = null;
        if(document.querySelector('#mp4_cast').checked) {
            mediaInfo = new chrome.cast.media.MediaInfo(
                this.mediaContents[mediaIndex].prog, 'video/mp4');
        } else {
            mediaInfo = new chrome.cast.media.MediaInfo(
                mediaIndex, 'application/x-mpegurl');
        }
...
CastPlayer.prototype.selectMedia = function(mediaIndex) {
    ...
    // Set video image
    var vi = document.getElementById('video_image');
    vi.src = this.mediaContents[mediaIndex]['poster'];
...
CastPlayer.prototype.initializeUI = function() {
    ...
    document.getElementById('media_title').innerHTML =
        this.mediaContents[this.currentMediaIndex]['title'];
    document.getElementById('media_subtitle').innerHTML =
        this.mediaContents[this.currentMediaIndex]['author'];
    document.getElementById('media_desc').innerHTML =
...
CastPlayer.prototype.addVideoThumbs = function() {
    ...
    var newdiv = null;
    var divIdName = null;
    for (var elem in this.mediaContents){

        if(!this.currentMediaIndex) {
            this.currentMediaIndex = elem;
        }

        newdiv = document.createElement('div');
        divIdName = 'thumb' + elem + 'Div';
        newdiv.setAttribute('id', divIdName);
        newdiv.setAttribute('class', 'thumb');
        newdiv.innerHTML =
            '<img src="' + this.mediaContents[elem]['poster'] +
            '" class="thumbnail">';
        newdiv.addEventListener('click', this.selectMedia.bind(this, elem));
